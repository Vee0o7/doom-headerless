...
//
// Animation.
// There is another anim_wi_t used in p_spec.
//
typedef struct
{
    animenum_t	type;
...
#define ANIM(type, period, nanims_wi, x, y, nexttic)            \
   { (type), (period), (nanims_wi), { (x), (y) }, (nexttic),    \
...
static int NUManims_wi[NUMEPISODES] =
{
...
{
    V_DrawPatchDiv2All(0, 0, background);
}
...
void WI_drawLF(void)
{
    ...
    {
        // draw <LevelName> 
        V_DrawPatchDiv2All((SCREENWIDTH - SHORT(lnames[wbs->last]->width))/2,
                    y, lnames[wbs->last]);

        // draw "Finished!"
        y += (5*SHORT(lnames[wbs->last]->height))/4;

        V_DrawPatchDiv2All((SCREENWIDTH - SHORT(finished->width)) / 2, y, finished);
    }
...
void WI_drawEL(void)
{
    ...
    // draw "Entering"
    V_DrawPatchDiv2All((SCREENWIDTH - SHORT(entering->width))/2,
		y,
                entering);
    ...
    V_DrawPatchDiv2All((SCREENWIDTH - SHORT(lnames[wbs->next]->width))/2,
		y, 
                lnames[wbs->next]);
  ...
  patch_t*	c[] )
{
    ...
    if (fits && i<2)
    {
	V_DrawPatchDiv2All(lnodes[wbs->epsd][n].x,
                    lnodes[wbs->epsd][n].y,
		    c[i]);
    }
    else
...
void WI_initAnimatedBack(void)
{
    ...
    for (i=0;i<NUManims_wi[wbs->epsd];i++)
    {
...
void WI_updateAnimatedBack(void)
{
	...
	if (bcnt == a->nexttic)
	{
	    ...
	    {
	      case ANIM_ALWAYS:
		if (++a->ctr >= a->nanims_wi) a->ctr = 0;
		a->nexttic = bcnt + a->period;
		break;

	      case ANIM_RANDOM:
		a->ctr++;
		if (a->ctr == a->nanims_wi)
		{
		    a->ctr = -1;
...
void WI_drawAnimatedBack(void)
{
	...
	if (a->ctr >= 0)
	    V_DrawPatchDiv2All(a->loc.x, a->loc.y, a->p[a->ctr]);
    }
  ...
  int		digits )
{
    ...
    {
	x -= fontwidth;
	V_DrawPatchDiv2All(x, y, num[ n % 10 ]);
	n /= 10;
    }

    // draw a minus sign if necessary
    if (neg)
	V_DrawPatchDiv2All(x-=8, y, wiminus);
  ...
  int		p )
{
    ...
    V_DrawPatchDiv2All(x, y, percent);
    WI_drawNum(x, y, p, -1);
}
  ...
  int		t )
{
    ...
    {
	// "sucks"
	V_DrawPatchDiv2All(x - SHORT(sucks->width), y, sucks); 
    }
}
...
void WI_drawStats(void)
{
    ...
    V_DrawPatchDiv2All(SP_STATSX, SP_STATSY, kills);
    WI_drawPercent(SCREENWIDTH - SP_STATSX, SP_STATSY, cnt_kills[0]);

    V_DrawPatchDiv2All(SP_STATSX, SP_STATSY+lh, items);
    WI_drawPercent(SCREENWIDTH - SP_STATSX, SP_STATSY+lh, cnt_items[0]);

    V_DrawPatchDiv2All(SP_STATSX, SP_STATSY+2*lh, sp_secret);
    WI_drawPercent(SCREENWIDTH - SP_STATSX, SP_STATSY+2*lh, cnt_secret[0]);

    V_DrawPatchDiv2All(SP_TIMEX, SP_TIMEY, timepatch);
    WI_drawTime(SCREENWIDTH/2 - SP_TIMEX, SP_TIMEY, cnt_time);

    if (wbs->epsd < 3)
    {
	V_DrawPatchDiv2All(SCREENWIDTH/2 + SP_TIMEX, SP_TIMEY, par);
...
static void WI_loadUnloadData(load_callback_t callback)
{
	...
	for (i=0 ; i<NUMMAPS ; i++)
	{
    b = sprint_str(name, "WILV");
    b = sprint_int(b, wbs->epsd, 0);
    b = sprint_int(b, i, 0);
            callback(name, &lnames[i]);
	}
	...
	if (wbs->epsd < 3)
	{
	    for (j=0;j<NUManims_wi[wbs->epsd];j++)
	    {
		a = &anims_wi[wbs->epsd][j];
		for (i=0;i<a->nanims_wi;i++)
		{
		    // MONDO HACK!
		    if (wbs->epsd != 1 || j != 8)
		    {
			// animations
    b = sprint_str(name, "WIA");
    b = sprint_int(b, wbs->epsd, 0);
    b = sprint_int(b, j, 2);
    b = sprint_int(b, i, 2);
                        callback(name, &a->p[i]);
    ...
    {
	// "1,2,3,4"
        b = sprint_str(name, "STPB");
        b = sprint_int(b, i, 0);
        callback(name, &p[i]);

	// "1,2,3,4"
        b = sprint_str(name, "WIBP");
        b = sprint_int(b, i+1, 0);
        callback(name, &bp[i]);
    }
